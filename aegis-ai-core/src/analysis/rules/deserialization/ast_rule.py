...
_SINKS: List[_SinkDef] = [
    _SinkDef("pickle",      "loads",      "Critical",
             "pickle.loads() 对不可信数据执行反序列化，可导致任意代码执行（RCE）。"
             "建议使用 JSON 替代，或在可信边界内使用 hmac 签名验证。",
             "CWE-502"),
    _SinkDef("pickle",      "load",       "Critical",
             "pickle.load() 可能加载不可信文件流，导致任意代码执行。",
             "CWE-502"),
    _SinkDef("marshal",     "loads",      "Critical",
             "marshal.loads() 不验证数据来源，反序列化不可信数据可导致 RCE。",
             "CWE-502"),
    _SinkDef("marshal",     "load",       "Critical",
             "marshal.load() 存在不可信数据反序列化风险。",
             "CWE-502"),
    _SinkDef("dill",        "loads",      "Critical",
             "dill.loads() 与 pickle 等效，反序列化不可信数据可导致 RCE。",
             "CWE-502"),
    _SinkDef("dill",        "load",       "Critical",
             "dill.load() 反序列化不可信数据，存在 RCE 风险。",
             "CWE-502"),
    _SinkDef("jsonpickle",  "decode",     "Critical",
             "jsonpickle.decode() 支持 Python 对象序列化，对不可信数据反序列化等同于 pickle。",
             "CWE-502"),
    _SinkDef("shelve",      "open",       "High",
             "shelve.open() 使用 pickle 作为后端，路径参数来自用户输入存在安全风险。",
             "CWE-502"),
    _SinkDef("yaml",        "load",       "High",
             "yaml.load() 未指定安全 Loader 时可执行任意代码。"
             "建议使用 yaml.safe_load() 或 yaml.load(data, Loader=yaml.SafeLoader)。",
             "CWE-502"),
    _SinkDef("yaml",        "safe_load",  "Low",
             "yaml.safe_load() 参数来自用户输入，建议验证数据来源。",
             "CWE-20"),
    _SinkDef("json",        "loads",      "Low",
             "json.loads() 参数来自用户输入（风险较低，但可能导致大 JSON 拒绝服务）。",
             "CWE-20"),
    _SinkDef(None,          "eval",       "Critical",
             "eval() 执行用户输入内容，等同于任意代码执行。",
             "CWE-95"),
    _SinkDef(None,          "exec",       "Critical",
             "exec() 执行用户输入内容，等同于任意代码执行。",
             "CWE-95"),
]

# 按 (module, func) 索引快速查找
_SINK_INDEX: Dict[tuple[Optional[str], str], _SinkDef] = {
    (s.module, s.func): s for s in _SINKS
}

# Python 用户输入结构化模式
_USER_INPUT_ATTRS = frozenset([
    "form", "args", "json", "data", "values", "cookies", "headers",
    "GET", "POST", "FILES", "params", "query_params", "body",
])
...
def _is_user_input_node(
    node: ast.AST, context: Optional[AnalysisContext] = None
) -> bool:
    """
    判断节点是否来自用户输入。
    ...
    if isinstance(node, ast.Name):
        lname = node.id.lower()
        for kw in ("user", "input", "request", "param", "arg",
                   "query", "form", "data", "body", "payload", "untrusted", "raw"):
            if kw in lname:
                return True
    return False
