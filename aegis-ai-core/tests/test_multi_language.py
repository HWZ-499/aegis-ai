...
from src.analysis.security_rules import VULN_SIGNATURES, scan_code_locally
...
def test_report_generation():
    ...
    try:
        ...
        if json_data.get("project_name") == "多语言测试项目" and results_count >= 3:
            ...
            print(f"❌ JSON 报告生成失败: project_name={json_data.get('project_name')}, findings_count={len(json_data.get('findings', {}))}")
            json_ok = False
    ...
    try:
        ...
        if "多语言测试项目" in html_report and ("<html>" in html_report or "<HTML>" in html_report or "DOCTYPE html" in html_report):
            print("✅ HTML 报告生成成功")
            html_ok = True
        else:
            print(f"❌ HTML 报告生成失败: 长度={len(html_report)}, 包含项目名={('多语言测试项目' in html_report)}, 包含html标签={('<html>' in html_report.lower())}")
            html_ok = False
    except Exception as e:
...
def main():
    ...
    results.append(("Python 检测", test_python_detection()))
    results.append(("JavaScript 检测", test_javascript_detection()))
    results.append(("Java 检测", test_java_detection()))
    results.append(("C/C++ 检测", test_cpp_detection()))
    results.append(("多语言项目扫描", test_multi_language_project_scan()))
