...
class MultiLanguageASTAnalyzer:
    ...
    def analyze(self, code_content: str, language: Optional[str] = None, 
    ...
    def _analyze_javascript(self, code_content: str, file_path: Optional[str] = None) -> List[Dict]:
        """
                ...
                if hasattr(left_node, 'text'):
                    ...
                    if 'innerHTML' in left_text or 'outerHTML' in left_text:
                        findings.append({
                            ...
                            "type": "XSS_RISK",
                            "severity": "High",
                            "details": f"JavaScript AST: 检测到 innerHTML/outerHTML 赋值操作",
                            "source": "AST"
                        })
    ...
    def _identify_caller_type(self, callee_node: Node, call_node: Node) -> str:
        ...
        if callee_node.type == 'member_expression':
            ...
            if obj_node:
                ...
                # 【P0优化】识别第三方库对象
                third_party_libs = ['THREE', 'jQuery', '$', 'React', 'Vue', 'Angular', 
                                   'Backbone', 'Underscore', 'Lodash', '_']
            ...
            if method_name == 'exec':
                ...
                if 'Runtime' in parent_text:
                    findings.append({
                        ...
                        "severity": "High",
                        "details": f"Java AST: 发现 Runtime.exec() 调用",
                        "source": "AST"
                    })
    ...
    def _analyze_php(self, code_content: str, file_path: Optional[str] = None) -> List[Dict]:
        """
        分析 PHP 代码
...
def analyze_code_multi_language(code_content: str, file_path: Optional[str] = None) -> List[Dict]:
    """
    多语言代码分析入口函数。
