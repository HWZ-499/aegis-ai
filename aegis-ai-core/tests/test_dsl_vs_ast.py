...
def _assert_pair(
    ...
    assert ast_detected == expect_finding, (
        f"[AST mismatch] {rel_path} expect={expect_finding}, got={ast_detected}, findings={ast_findings}"
    )
    assert dsl_detected == expect_finding, (
        f"[DSL mismatch] {rel_path} expect={expect_finding}, got={dsl_detected}, findings={dsl_findings}"
    )
