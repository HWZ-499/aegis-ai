...
It turns existing Aegis security capabilities into OpenAI-style tool schemas and runs the local implementations through a controlled LangGraph workflow:

```text
scan -> analyze -> triage ─┬─> retrieve -> fix -> review -> summarize
                           └─> summarize (partial/error or no actionable findings)
```
...
The bundled knowledge retrieval is intentionally small, deterministic, and offline. It ranks curated Markdown documents by lexical overlap; it is not an embedding database. The workflow is also deterministic rather than an autonomous planning agent, which keeps file mutation and scan behavior predictable.
