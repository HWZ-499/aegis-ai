...
from src.scanner.taint_enhancer import TaintEnhancer, enhance_scan_results, TAINT_ANALYSIS_AVAILABLE
...
示例:
  ...
  python -m src.scanner.cli . --format sarif --output results.sarif
    ...
    parser.add_argument(
        '--ignore',
        nargs='+',
        help='要忽略的目录/文件模式（例如: --ignore node_modules .git）'
    )
    
    parser.add_argument(
        '--incremental', '-i',
        action='store_true',
        help='增量扫描模式：只扫描修改的文件（需要 Git 仓库）'
    )
    
    parser.add_argument(
        '--base-ref',
        type=str,
        help='增量扫描的 Git 基准引用（如 main, HEAD~1），默认扫描工作区更改'
    )
    ...
    parser.add_argument(
        '--max-workers',
        type=int,
        help='最大工作线程/进程数（默认 CPU 核心数）'
    )
    
    parser.add_argument(
        '--rag',
        action='store_true',
        help='启用 RAG 增强：为扫描结果添加修复建议和 CVE 关联'
    )
    
    parser.add_argument(
        '--rag-db',
        type=str,
        help='RAG 知识库路径（ChromaDB 数据库目录）'
    )
    
    parser.add_argument(
        '--ai',
        action='store_true',
        help='启用 AI 分析：对高风险发现进行 AI 验证（需要 API 密钥）'
    )
    
    parser.add_argument(
        '--taint',
        action='store_true',
        help='启用污点分析：追踪 Source → Sink 数据流路径（P3 高级功能）'
    )
    
    parser.add_argument(
        '--cross-file',
        action='store_true',
        help='启用跨文件分析：追踪模块间依赖和数据流（P4 高级功能）'
    )
                        ...
                        if taint_results:
                            ...
                            for tf in taint_results:
                                ...
                                for existing in findings:
                                    if existing.get('line') == tf.get('line') and \
                                       existing.get('type') == tf.get('type'):
                                        is_duplicate = True
                                        # 更新现有发现的污点信息
