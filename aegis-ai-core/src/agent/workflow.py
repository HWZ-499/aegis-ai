...
def _analyze_node(state: AgentState) -> AgentState:
    ...
    _trace(next_state, "analyze", "load_project_memory", started, detail=str(memory.get("baseline_path", "")))
    ...
    for finding in next_state.get("findings", []):
        ...
        _trace(next_state, "analyze", "get_finding_detail", started, detail=str(detail.get("finding_id", "")))
...
def _triage_node(state: AgentState) -> AgentState:
    ...
    if partial or error_count:
        for finding in grouped_findings:
            finding["diagnosis_status"] = "unverified_partial_scan"
            finding["review_note"] = (
                "This finding came from a partial scan. Review the scan errors before trusting or fixing it."
            )
        plan = {
            "mode": "scan_error",
            "route": "summarize",
            "reason": "Scan produced partial results or errors; findings are retained but remediation is skipped.",
        ...
        _trace(next_state, "triage", "plan_next_step", time.perf_counter(), detail=str(plan["reason"]))
        return next_state
