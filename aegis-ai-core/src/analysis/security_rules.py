...
VULN_SIGNATURES_COMPILED = {
    k: [re.compile(r, re.IGNORECASE) for r in v] for k, v in VULN_SIGNATURES.items()
}
...
_RE_PHP_VALIDATION    = re.compile(
    r'\b(is_numeric|intval|ctype_digit|preg_match|filter_var|in_array|array_search|strip_tags)\s*\(',
    re.IGNORECASE
)
        ...
        if '//' in line or '#' in line:
            ...
            for i, char in enumerate(line):
                if char == "'" and (i == 0 or line[i-1] != '\\'):
                    ...
                    in_double_quote = not in_double_quote
                elif (char == '#' or (i < len(line) - 1 and line[i:i+2] == '//')) and not in_single_quote and not in_double_quote:
                    comment_pos = i
                    break
            if comment_pos >= 0:
                line = line[:comment_pos]
...
class PhpTaintGraph:
    ...
    def _build(self) -> None:
        ...
        for idx, raw_line in enumerate(self._lines):
            ...
            if _PHP_SANITIZE_RE.search(expr):
                ...
                inner_tainted = bool(
                    (inner and inner in self._tainted_vars)
                    or _PHP_SOURCE_RE.search(expr)
                )
                self._var_map[var] = {"line": idx + 1, "expr": expr,
    ...
    _GUARD_FUNC_NAMES: frozenset[str] = frozenset({
        "is_numeric", "intval", "ctype_digit", "ctype_alpha", "ctype_alnum",
        "is_int", "is_float", "is_string", "filter_var", "preg_match", "in_array",
    })
            ...
            for pat in compiled_list:
                ...
                if pat.search(match_line):
                    ...
                    if vuln_type == "RCE_COMMAND_EXEC" and language in ["javascript", "typescript"]:
                        ...
                        if re.search(r'(new\s+\w*[Ss]ystem|var\s+\w*[Ss]ystem|let\s+\w*[Ss]ystem|const\s+\w*[Ss]ystem|class\s+\w*[Ss]ystem)', code_only_line, re.IGNORECASE):
                            continue  # 跳过类名/变量名
                        ...
                        if re.search(r'\bsystem\s*\(', code_only_line, re.IGNORECASE):
                            if 'child_process' not in code_only_line and 'process' not in code_only_line:
                                ...
                                if 'child_process' not in context_lines.lower() and 'process' not in context_lines.lower():
                    ...
                    if vuln_type == "PATH_TRAVERSAL" and language in ["javascript", "typescript"]:
                        ...
                        if not any(re.search(pattern, context_lines, re.IGNORECASE) for pattern in file_operation_patterns):
                    ...
                    if vuln_type == "DESERIALIZATION":
                        ...
                        if language in ["javascript", "typescript"]:
                            ...
                            if re.search(r'JSON\.parse\s*\(\s*(req\.body|body|request\.body)', code_only_line, re.IGNORECASE):
                                # 检查上下文是否包含Express/Node.js关键词
                                express_keywords = ['app.', 'router.', 'express.', 'req.', 'res.', 'next(', 'middleware', 
                                                  'app.use', 'app.post', 'app.get', 'router.post', 'router.get']
                            ...
                            if re.search(r'(var|let|const)\s+\w+\s*=\s*JSON\.parse', code_only_line, re.IGNORECASE):
                                ...
                                if not any(re.search(pattern, code_only_line, re.IGNORECASE) for pattern in untrusted_patterns):
                                    # 不是处理不可信数据，跳过或降级
                            ...
                            if re.search(r'JSON\.parse\s*\(\s*(localStorage|sessionStorage)', code_only_line, re.IGNORECASE):
                                continue  # 完全跳过，不是漏洞
                            ...
                            if not any(re.search(pattern, code_only_line, re.IGNORECASE) for pattern in untrusted_sources):
                                # 不是处理不可信数据，跳过
                                continue
                    ...
                    if vuln_type == "RCE_COMMAND_EXEC":
                        ...
                        if file_path:
                            ...
                            _setup_names = ('setup', 'install', 'migrate', 'upgrade', 'seed',
                                            'bootstrap', 'fixture', 'deploy', 'init')
                            if any(_basename.startswith(n) or _basename == n + '.php'
                                   for n in _setup_names):
                        ...
                        if language in ["javascript", "typescript"]:
                            ...
                            if re.search(r'\w+\.exec\s*\(', code_only_line):
                                ...
                                if var_match:
                                    ...
                                    if any(keyword in var_name for keyword in ['regex', 'pattern', 'reg', 'match', 'color', 'rgb']):
                            ...
                            if re.search(r'^\s*\w+\.exec\s*\(', code_only_line):
                                # 检查是否是真正的命令执行函数
                                if not re.search(r'child_process|shell_exec|Runtime\.getRuntime|ProcessBuilder', code_only_line, re.IGNORECASE):
                                    # 可能是 RegExp.exec()，跳过
                                    continue
                        ...
                        if _RE_PHP_USER_INPUT.search(raw):
                            ...
                            if _var_m:
                                ...
                                _user_assign = re.search(
                                    rf'{_var}\s*=\s*.*\$_(GET|POST|REQUEST|COOKIE)',
                                    _ctx, re.IGNORECASE
                                )
                                if not _user_assign:
