...
| **O5** | 增量扫描性能优化 | **已完成（基础验收）** | Python/JS/TS 顶层函数体变更走 sparse partial scan；导出签名变化会触发导入方重扫；1002 行 fake LSP didChange 核心路径 30 次测量 p95 103.96ms / max 113.30ms |
...
| **O5** | 增量扫描优化 | 🟧 高 — 大项目 | 大（1-2 周） | **P1** | 第 2 阶段 | 已完成（基础验收） |
...
- [x] 单文件编辑后 didChange 诊断延迟 < 500ms（1000 行文件；2026-06-12 本机 fake LSP `_validate_document` 1002 行样本 30 次函数体修改：median 88.48ms、p95 103.96ms、max 113.30ms）
- [x] 仅修改函数体时只重分析该函数（Python / JS / TS 顶层函数先支持，其他语言保守回退全量扫描）
- [x] 导出函数签名变化时自动重扫依赖文件
- [x] 性能基准测试加入 CI（`pytest-benchmark`）
...
| 日期 | 变更内容 |
|------|----------|
| 2026-06-12 | O5 基础验收完成：1002 行 fake LSP didChange 核心路径 30 次测量全部低于 500ms，median 88.48ms / p95 103.96ms / max 113.30ms |
| 2026-06-12 | O5 第一批推进：接入 Python/JS/TS 顶层函数 sparse partial scan、缓存 finding 行号重映射、导出签名变化后的导入方重扫 |
