...
class SecurityVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        # 2. 检测 subprocess.call / os.system (命令注入)
            ...
            try:
                ...
                elif module_name == 'subprocess' and func_name in ['call', 'run', 'Popen', 'check_call', 'check_output']:
                    self.issues.append({
                        "line": node.lineno,
                        "type": "RCE_COMMAND_EXEC",
    ...
    def visit_Assign(self, node):
        ...
        if isinstance(node.targets[0], ast.Name):
            ...
            # 检测硬编码密码、密钥
            if any(keyword in var_name for keyword in ['password', 'passwd', 'pwd', 'secret', 'key', 'token', 'api_key', 'apikey']):
