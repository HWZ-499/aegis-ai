...
def merge_findings(ast_findings: list[dict], regex_findings: list[dict]) -> list[dict]:
    ...
    for finding in ast_findings:
        ...
        findings_dict[key] = {
            ...
            **{k: v for k, v in finding.items() if k not in ("line", "type", "severity", "details")},
        }
    ...
    for finding in regex_findings:
        ...
        if not severity:
            ...
            severity = "High" if "High" in confidence else "Medium" if "Medium" in confidence else "Low"
        ...
        if key not in findings_dict:
            findings_dict[key] = {
                ...
                "source": "Regex",
                **{k: v for k, v in finding.items() if k not in ("line", "type", "severity", "details", "content")},
            }
        else:
