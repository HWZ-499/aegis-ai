...
class RuleConfig:
    ...
    def get_enabled_signatures(self) -> dict[str, list[str]]:
        ...
        for vuln_type, patterns in VULN_SIGNATURES.items():
            enabled_patterns = [pattern for pattern in patterns if self.is_rule_enabled(vuln_type, pattern)]
...
def create_default_config(output_path: str) -> Path:
    ...
    default_config = {
        ...
        "version": "1.0",
        "enabled_rules": {"comment": "启用列表为空 {} 表示启用所有规则", "example": {}},
        "disabled_rules": {
            "comment": "禁用特定规则，格式: {漏洞类型: [规则模式列表]}",
            "example": {"RCE_COMMAND_EXEC": ['r"os\\.system\\s*\\("']},
        },
        "custom_rules": {
            "comment": "自定义规则，格式: {漏洞类型: [规则模式列表]}",
            "example": {"CUSTOM_VULN": ['r"custom_pattern"']},
        },
    }
