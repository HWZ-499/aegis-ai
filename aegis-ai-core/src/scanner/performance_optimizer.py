...
class ScanCache:
    ...
    def get_cached_result(self, file_path: Path) -> Optional[List[Dict]]:
        """
        获取缓存的扫描结果
...
class ParallelScanner:
    ...
    def scan_files_parallel(self, file_paths: List[Path], 
                           ...
                           progress_callback: Optional[callable] = None) -> Dict[Path, List[Dict]]:
        """
        并行扫描多个文件
        ...
        # 准备参数元组
        args_list = [
            (file_path, str(project_path), supported_extensions)
...
class PerformanceOptimizer:
    ...
    def scan_files_optimized(self, file_paths: List[Path],
                             scan_func: callable,
                             project_path: Path,
                             supported_extensions: Dict[str, str],
                             progress_callback: Optional[callable] = None,
                             engine: str = "legacy") -> Dict[Path, List[Dict]]:
        """
        优化的文件扫描（使用缓存和并行处理）
