...
检测目标：
- 用户可控输入流入 HttpServletResponse.sendRedirect 等重定向 API；
- 依赖统一污点图 TaintGraph 与 Java Sink 注册（OPEN_REDIRECT 类别）。
"""
...
class JavaOpenRedirectAstRule(SecurityRule):
    """
    基于 TaintGraph 的 Java Open Redirect 检测规则。
    """
    ...
    def after_file(self, context: AnalysisContext) -> None:
        ...
        for path in paths:
            ...
            details = (
                "检测到 Java 代码中用户可控输入直接用于构造重定向目标（response.sendRedirect），"
