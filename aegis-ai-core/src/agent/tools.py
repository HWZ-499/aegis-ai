...
def group_findings_for_report(findings: list[Finding]) -> tuple[list[Finding], dict[str, int]]:
    ...
    Findings on the same line are treated as duplicate detector evidence. Nearby
    findings are grouped only when they come from different rule ids; two sinks
    reported by the same detector must remain separate even when they are close.
    """
    ...
    for finding in sorted(
        ...
        for group in grouped:
            ...
            if line in group_lines:
                target_group = group
                break
            group_rule_ids = {_finding_rule_id(item) for item in group}
            if (
                window > 0
                and _finding_rule_id(finding) not in group_rule_ids
                and min(abs(line - group_line) for group_line in group_lines) <= window
            ):
                target_group = group
                break
