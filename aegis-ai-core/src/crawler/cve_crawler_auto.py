...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('cve_crawler.log'),
...
class CVECrawler:
    ...
    def fetch_cves_from_nvd(self, start_index: int = 0, results_per_page: int = 20,
                           pub_start_date: Optional[str] = None, pub_end_date: Optional[str] = None,
    ...
    def crawl_recent_cves(self, days: int = 7, max_results: int = 100, keyword: Optional[str] = None) -> List[Dict]:
        """
        爬取最近 N 天的 CVE 数据
        ...
        if keyword:
            ...
            logger.info(f"🔍 开始爬取最近 {days} 天的 CVE 数据（从 {start_date.strftime('%Y-%m-%d')} 到 {end_date.strftime('%Y-%m-%d')}）...")
